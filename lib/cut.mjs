...
const SD_PER_HOLE = 0.62; // typical spread of a golfer's score on one hole, in strokes
...
export function cutByManager(leaderboard, cut) {
  ...
  return leaderboard.teams
    .map((t) => {
      const golfers = t.lineup.map((g) => ({ name: g.name, toPar: g.teamToPar, status: g.status, thru: g.thru, pct: pct(g), sub: !!g.subFor }));
