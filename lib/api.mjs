...
import { fetchEspn, parseEspn, parseHoles } from './espn.mjs';
...
export function buildFieldBoard(d, feed) {
  const starters = d.settings?.starters ?? DEFAULT_SETTINGS.starters;
  const byId = new Map();
  const byKey = new Map();
  for (const p of d.picks || []) {
    const owner = { managerId: p.managerId, round: p.round, backup: p.round > starters, pickKey: p.key };
  ...
  for (const p of d.picks || []) {
    ...
    rows.push({ key: p.key, name: p.name, espnId: p.espnId || null, pos: '-', status: state === 'pre' ? 'active' : 'wd', toPar: null, today: '', thru: '', teeTime: p.teeTime || null, rounds: [null, null, null, null], sortOrder: 10000 + p.n, owner: { managerId: p.managerId, round: p.round, backup: p.round > starters, pickKey: p.key } });
  }
  ...
  try {
    ...
    if (parts[0] === 'draft' && parts[1]) {
      ...
      if (method === 'GET' && parts[2] === 'field') {
        ...
        return json({ ...buildFieldBoard(d, feed), error, fetchedAt });
      }
      ...
      if (method === 'GET' && parts[2] === 'scorecard') {
        ...
        if (!rounds || !rounds.some(Boolean)) return json({ name: pick.name, available: false, reason: data ? 'No holes played yet.' : "ESPN hasn't posted hole-by-hole scores for this tournament." });
    ...
    if (parts[0] === 'admin' && parts[1] === 'draft' && parts[2]) {
      ...
      const next = await mutateDraft(id, async (d) => {
        switch (action) {
          ...
          case 'replace-pick': {
            const p = d.picks.find((x) => x.n === body.n);
            if (!p) fail(404, 'Pick not found.');
            const g = d.field.find((f) => f.key === body.golferKey);
            if (!g) fail(400, 'Golfer not in field.');
            if (d.picks.some((x) => x.key === g.key && x.n !== p.n)) fail(409, `${g.name} is already taken.`);
