...
import { fetchEspn, parseEspn } from './espn.mjs';
        ...
        switch (action) {
          ...
          case 'field': {
            ...
            if (body.source === 'espn') {
              ...
              entries = feed.golfers.map((g) => ({ name: g.name, key: g.key, espnId: g.espnId, teeTime: g.teeTime }));
            } else {
              entries = String(body.names || '').split(/\r?\n|,(?=\s*[A-Z])/).map((s) => s.replace(/\t.*$/, '').trim()).filter(Boolean).map((name) => ({ name, key: nameKey(name), espnId: null, teeTime: null }));
            }
            const map = new Map((body.mode === 'append' ? d.field : []).map((f) => [f.key, f]));
            for (const e of entries) map.set(e.key, { ...map.get(e.key), ...e });
            // keep drafted golfers even if they drop out of the new list
            for (const p of d.picks) if (!map.has(p.key)) map.set(p.key, { name: p.name, key: p.key, espnId: p.espnId, teeTime: p.teeTime });
            d.field = [...map.values()];
